...
struct minimum_space_impl<MemorySpace,MemorySpace> {
    typedef MemorySpace type;
};
template <typename MemorySpace>
struct minimum_space_impl<any_memory,MemorySpace>  {
    typedef MemorySpace type;
};
template <typename MemorySpace>
struct minimum_space_impl<MemorySpace,any_memory>  {
    typedef MemorySpace type;
};
template <>
struct minimum_space_impl<any_memory,any_memory>   {
    typedef any_memory  type;
};
...
struct minimum_space {
    typedef typename detail::minimum_space_impl< typename detail::minimum_space_impl<MemorySpace1,MemorySpace2>::type, MemorySpace3>::type type;
};
