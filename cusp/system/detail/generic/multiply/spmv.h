              ...
              array1d_format)
{
    ...
    RowIndexIterator row_indices_begin(CountingIterator(0), divide_value<IndexType>(A.values.num_cols));

    ModulusIterator gather_indices_begin(CountingIterator(0), modulus_value<IndexType>(A.values.num_cols));
    ...
    PermIndexIterator   perm_indices_begin(CountingIterator(0),   PermFunctor(A.values.num_rows, A.values.num_cols, A.values.pitch));
              ...
              array1d_format)
{
    ...
    // define types used to programatically generate row_indices
    typedef typename thrust::counting_iterator<IndexType>                                  CountingIterator;
    typedef typename thrust::transform_iterator<LogicalFunctor, CountingIterator>          StrideIndexIterator;
    typedef typename thrust::transform_iterator<divide_value<IndexType>, CountingIterator> RowIndexIterator;
    ...
    RowIndexIterator row_indices_begin(CountingIterator(0), divide_value<IndexType>(num_entries_per_row));
    StrideIndexIterator stride_indices_begin(CountingIterator(0), logical_functor);
    cusp::detail::temporary_array<ValueType, MemorySpace, DerivedPolicy> vals(exec, A.num_rows);
              ...
              array1d_format)
{
    ...
    cusp::offsets_to_indices(thrust::detail::derived_cast(exec), A.row_offsets, row_indices);
